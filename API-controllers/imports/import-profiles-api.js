...
router.get('/api/getProfile/:ident_code', (req, res) => {
    const identCode = req.params.ident_code;
    const sql = `SELECT profile_id, metering_type FROM clients WHERE ident_code = REPLACE('${identCode.trim()}', '"','')`;
    db.query(sql, (err, result) => {
        if (err) {
            throw err;
        }
