...
router.post('/api/addExchangePriceReadings', async (req, res) => {
    ...
    if (exchangeRatesReadings != [] && exchangeRatesReadings != undefined && exchangeRatesReadings.length && exchangeRatesReadings != null) {
       ...
       created_date = VALUES(created_date)`;
        db.query(sql, [exchangeRatesReadings], (err, result) => {
            if (err) {
                throw err;
            }
            console.log('Данните с борсовите цени са импортирани');
            return res.send("Данните с борсовите цени са импортирани");
        });
    } else {
...
async function filterExchangePriceReadings(allExchangePriceReadings) {
    let readingsFiltered = [];
    let addToFinalReadings;
    for (let i = 0; i < allExchangePriceReadings.length; i += 1) {
        addToFinalReadings = true;
        let currHourReading = allExchangePriceReadings[i];
        ...
        const createdDate = currHourReading[2];
        ...
        /* let selectReading = `SELECT * FROM exchange_prices 
        WHERE date = '${currDate}'
        AND type = '${typeEnergy}'
        LIMIT 1`;
