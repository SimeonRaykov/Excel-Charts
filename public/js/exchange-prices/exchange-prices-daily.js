...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
        eventLimit: true,
        eventLimit: 1,
        eventLimitText: 'Има цена',
        eventLimitClick: 'day',
        allDaySlot: false,
        eventOrder: 'groupId',
        defaultDate: fixedDate,
        events: getExchangePricesDailyData(),
        plugins: ['timeGrid', ],
...
function getExchangePricesDailyData() {
    let currDate = findGetParameter('date'),
        currHourReadingId = findGetParameter('id');;
    let dataArr = [];
    $.ajax({
        url: `/api/exchange-prices/daily/${currHourReadingId}/${currDate}`,
        method: 'GET',
        dataType: 'json',
        async: false,
...
function showChartDaily(data) {
    ...
    var ctx = document.getElementById('daily-exchange-prices-chart').getContext('2d');
    var config = {
        type: 'line',
        data: {
            labels,
        ...
        for (let [key, value] of Object.entries(data[el])) {
            ...
            if (i >= startIndex && i <= endIndex) {
                currHourReading = {
                    groupId: key,
                    id: key,
                    title: value === null ? title = 'Няма стойност' : `Стойност: ${Number(value)}`,
...
function processDateHeading(date) {
    ...
    writeExchangePricesDateHeading(formattedDate);
}

function writeExchangePricesDateHeading(date) {
    $('#date-heading').text(`Борсови цени за дата: ${date}`);
}
